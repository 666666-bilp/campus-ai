      ...
      <main class="flex-1 p-6 max-w-4xl mx-auto w-full">
        ...
        <template v-else>
          <div class="card mb-6">
            ...
            <div class="flex flex-wrap gap-2"><button @click="generateFull" :disabled="genLoading" class="btn-primary btn-sm"><span v-if="genLoading" class="inline-block w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin mr-1"></span>生成全文</button><button @click="exportPaper('docx')" class="btn-primary btn-sm">导出Word</button><button @click="exportPaper('txt')" class="btn-secondary btn-sm">导出TXT</button><button @click="exportPaper('md')" class="btn-secondary btn-sm">导出MD</button><button @click="streamGenerate" :disabled="streamLoading" class="btn-outline btn-sm">流式生成</button><button @click="delPaper" class="btn-danger btn-sm">删除</button></div>
          </div>
